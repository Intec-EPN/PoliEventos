...
export const EditarCategorias = () => {
  ...
  const onNuevaCategoria = () => {
    const nuevosTipos = watch("nuevosTipos") || []; // Asegúrate de obtener un array válido.

    // Verificar si el array está vacío
    if (nuevosTipos.length === 0) {
      console.log("El array está vacío. Se permite el ingreso.");
      // Aquí permites el ingreso, podrías retornar o realizar la acción de agregar.
      return;
    }

    // Verificar si el último tipo es vacío
    const ultimoTipo = nuevosTipos[nuevosTipos.length - 1];
    if (!ultimoTipo || ultimoTipo.trim() === "") {
      console.log("nuevosTipos no está definido o el último tipo está vacío.");
      return;
    }

    // Si todo está bien, puedes agregar una nueva categoría.
    console.log("Permitir agregar nueva categoría.");
  };
