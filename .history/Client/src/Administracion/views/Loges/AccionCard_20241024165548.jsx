...
import { Box, Card, CardContent, Typography, Tooltip, Grid, Avatar } from '@mui/material';
...
const AccionCard = ({ accion }) => {
  ...
  return (
    <Card sx={{ display: 'flex', my: 1, borderRadius: '12px', border: `1px solid ${accionData.bgColor}`, padding: '8px' }}>
      <Grid container>
        <Grid item xs={3} sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center', backgroundColor: accionData.bgColor, color: 'white', borderRadius: '8px' }}>
          <Typography variant="body1">{accionData.accion}</Typography>
        </Grid>
        <Grid item xs={9} sx={{ display: 'flex', alignItems: 'center', paddingLeft: '12px' }}>
